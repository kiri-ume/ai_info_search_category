...
- **2025-12-22 Ver0.1.2**:
    - **モデル情報記録**: 要約に使用したLLMモデル名をデータベースに保存・ログ出力する機能を追加。
    - **設定の外部化**: LLMのパラメータ設定を `llm_config.json` に分離し、モデルごとのチューニングを容易化。
    - **Qwen3対応**: ローカル軽量モデル **Qwen3-8B-MLX** での動作確認と専用設定の追加。
    - **解析処理の強化**: チャット形式のモデルが返す不要な会話文を除去し、JSONのみを抽出するパーサーの改善。
